...
from app.routes.users.controller import get_user_by_mail
from app.util.db_dependency import get_db
from sqlalchemy.orm import Session
from .schemas import TokenData
from app.routes.users.schemas import User
from app.routes.users.models import User as UserModel
from .models import PasswordResetToken
# from app.util.mail.mail_sender import send_password_reset_mail
from app.routes.users.controller import check_user_existence_by_id
from pydantic import EmailStr

# Generate secret key with "openssl rand -hex 32" (also described in FastAPI docs)
SECRET_KEY = ""
...
def authenticate_user(email: EmailStr, password: str, db: Session) -> User:
    ...
    user = get_user_by_mail(mail=email, db=db)
    if not user:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Could not get user with mail \"{email}\".",
...
async def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise credentials_exception
    user = get_user_by_mail(mail=token_data.email, db=db)
    if user is None:
        raise credentials_exception
    return user
