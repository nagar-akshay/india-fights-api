...
from app.routes.auth.schemas import Token, EmailSchema, SetNewPassword, UserLoginForm
from app.util.db_dependency import get_db
from app.routes.users.controller import check_user_existence_by_email
...
async def login_for_access_token(form_data: UserLoginForm, db: Session = Depends(get_db)):
    """
    # Login
    ...
    **Access:** Public.
    ...
    if not check_user_existence_by_email(mail=form_data.email, db=db):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="There is no user with this email."
        )

    if is_user_disabled(user_email=form_data.email, db=db):
        raise HTTPException(
